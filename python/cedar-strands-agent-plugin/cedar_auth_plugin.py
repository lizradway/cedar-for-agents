...
class CedarPolicyVerifier:
    ...
    def _schema_to_context(self, schema: dict[str, Any]) -> dict[str, str]:
        ...
        for name, prop in properties.items():
            ...
            # Non-required params are optional in context (tool may not pass them)
            # But Cedar doesn't have optional record fields outside entities,
            # so we include all params — the plugin only passes scalar values anyway
    ...
    def verify(self, policies: str | Path) -> VerificationResult:
        ...
        for tool_name in self._tools:
            ...
            if action_ref not in policies:
                # Check if there's a wildcard permit (action, not action == ...)
                # A policy with just "action," (no ==) covers all tools
    ...
    def _has_wildcard_action(policies: str) -> bool:
        ...
        import re
        # Match "permit(...action,...)" where action is NOT followed by == or in
        # This is imperfect but catches the common "permit(principal in Role::X, action, resource)" pattern
